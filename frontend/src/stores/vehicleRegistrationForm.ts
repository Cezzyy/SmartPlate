...
import { VehicleRegistrationState } from '../types/vehicleRegistrationFormState'
...
export const useVehicleRegistrationFormStore = defineStore('vehicleRegistrationForm', {
  ...
  state: (): VehicleRegistrationState => ({
    ...
    formData: {
      ...
      appointmentTime: '',
      referenceCode: '',
      inspectionStatus: 'pending' as RegStatus,
      inspectionCode: '',
      inspectionNotes: '',
      ...
      paymentStatus: 'pending' as RegStatus,
      paymentCode: '',
      paymentNotes: '',
      paymentDetails: {
        amountPaid: 0,
      ...
      verificationStatus: 'pending' as RegStatus,
      status: 'pending' as RegStatus,
      submissionDate: '',
      expiryDate: '',
      plateNumber: '',
      plateType: '',
      ...
      referenceSlip: '',
      submission: ''
    } as unknown as VehicleRegistrationForm,
    errors: {
      vehicleType: '',
      make: '',
      model: '',
  ...
  actions: {
    async fetchForms() {
      ...
      this.error = null
      try {
        const forms = await vehicleRegistrationService.getAllRegistrationForms()
        this.forms = forms
      } catch (error: any) {
    ...
    validateVehicleInfo(): boolean {
      ...
      if (!this.formData.engineNumber) {
        ...
        isValid = false
      } else if (this.formData.engineNumber.length < 5) {
        this.errors.engineNumber = 'Engine number must be at least 5 characters'
        isValid = false
      } else {
        // Check for existing engine number in other forms
        const existingForm = this.forms.find(
          form => form.engineNumber === this.formData.engineNumber && form.id !== this.formData.id
        )
        if (existingForm) {
          this.errors.engineNumber = 'This engine number is already registered'
          isValid = false
        }
      }
      ...
      if (!this.formData.chassisNumber) {
        ...
        isValid = false
      } else if (this.formData.chassisNumber.length < 5) {
        this.errors.chassisNumber = 'Chassis number must be at least 5 characters'
        isValid = false
      } else {
        // Check for existing chassis number in other forms
        const existingForm = this.forms.find(
          form => form.chassisNumber === this.formData.chassisNumber && form.id !== this.formData.id
        )
        if (existingForm) {
          this.errors.chassisNumber = 'This chassis number is already registered'
          isValid = false
        }
      }
    ...
    async submitForm() {
      if (!this.validateVehicleInfo() || !this.validateDocuments()) {
        return
      }
      ...
      try {
        ...
        const paymentResponse = await fetch('/api/payment/initialize', {
          ...
          body: JSON.stringify({
            amount: this.calculateTotalFees(),
            currency: 'USD',
            description: 'Vehicle Registration Fees',
            metadata: {
              registrationId: this.formData.referenceCode,
        ...
        this.formData.referenceCode = data.referenceNumber // Changed referenceNumber to referenceCode
        this.currentStep = 4
      } catch (error) {
        console.error('Error submitting form:', error)
        this.errors.submission = 'Failed to submit registration. Please try again.'
      } finally {
    ...
    calculateTotalFees(): number {
      ...
      if (this.formData.vehicleType === 'used') {
        total += 50 // Additional fee for used vehicles
      }
    ...
    validateDocuments(): boolean {
      ...
      // Reset errors
      this.errors.ownershipDocument = ''
      this.errors.insuranceDocument = ''
      this.errors.emissionTest = ''
      this.errors.vehicleInspection = ''

      // Ownership document validation
      if (!this.formData.ownershipDocument) {
        this.errors.ownershipDocument = 'Ownership document is required'
        isValid = false
      }

      // Insurance document validation
      if (!this.formData.insuranceDocument) {
        this.errors.insuranceDocument = 'Insurance document is required'
        isValid = false
      }

      // Additional validation based on vehicle type
      if (this.formData.vehicleType === 'used') {
        // Used vehicles require emission test
        if (!this.formData.emissionTest) {
          this.errors.emissionTest = 'Emission test certificate is required for used vehicles'
          isValid = false
        }

        // Used vehicles require vehicle inspection
        if (!this.formData.vehicleInspection) {
          this.errors.vehicleInspection = 'Vehicle inspection certificate is required for used vehicles'
          isValid = false
        }
      }
      ...
      type DocumentKey = 'ownershipDocument' | 'insuranceDocument' | 'emissionTest' | 'vehicleInspection'
      const documents: Array<{ name: DocumentKey; file: File | null }> = [
        { name: 'ownershipDocument', file: this.formData.ownershipDocument },
        { name: 'insuranceDocument', file: this.formData.insuranceDocument },
        { name: 'emissionTest', file: this.formData.emissionTest },
        { name: 'vehicleInspection', file: this.formData.vehicleInspection }
      ]

      for (const doc of documents) {
        if (doc.file && !validDocumentTypes.includes(doc.file.type)) {
          this.errors[doc.name] = 'Document must be in PDF, JPEG, or PNG format'
          isValid = false
        }

        if (doc.file && doc.file.size > 5 * 1024 * 1024) {
          this.errors[doc.name] = 'Document size must be less than 5MB'
          isValid = false
        }
      }
    ...
    transferToVehicleRegistration(formId: string): boolean {
      ...
      try {
        ...
        const vehicleData = {
          ...
          VEHICLE_SERIES: form.model,
          YEAR_MODEL: form.year.toString(),
          ENGINE_NUMBER: form.engineNumber,
          CHASSIS_NUMBER: form.chassisNumber,
          COLOR: form.color,
          VEHICLE_TYPE: form.vehicleType,
          LTO_CLIENT_ID: form.userId,
          status: 'Active'
        }
          ...
          .then(async (newVehicle) => {
            ...
            if (form.plateNumber) {
              const plateData = {
                ...
                plate_issue_date: form.plateIssueDate,
                plate_expiration_date: form.plateExpirationDate,
