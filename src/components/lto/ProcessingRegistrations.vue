...
const emit = defineEmits<{
  (e: 'inspection-process', registration: VehicleRegistrationForm): void
  (e: 'payment-process', registration: VehicleRegistrationForm): void
}>()
...
// Determine which step a registration is in
const isInspectionStep = (registration: VehicleRegistrationForm) => {
  return registration.inspectionStatus === 'pending'
}

const isPaymentStep = (registration: VehicleRegistrationForm) => {
  return registration.inspectionStatus === 'approved' && registration.paymentStatus === 'pending'
}
</script>
            ...
            <tr v-if="filteredRegistrations.length === 0" class="hover:bg-gray-50">
              <td colspan="6" class="px-6 py-10 text-center text-gray">
                <div class="flex flex-col items-center justify-center space-y-3">
                  ...
                  <p class="text-lg font-medium text-dark-blue">
                    No processing registrations found
                  </p>
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  ...
                  :class="{
                    'bg-yellow-100 text-yellow-800': isInspectionStep(registration),
                    'bg-blue-100 text-blue-800': isPaymentStep(registration),
                  }"
                >
                  {{ isInspectionStep(registration) ? 'Awaiting Inspection' : 'Awaiting Payment' }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right space-x-2">
                <button
                  v-if="isInspectionStep(registration)"
                  @click="emit('inspection-process', registration)"
                ...
                <button
                  v-if="isPaymentStep(registration)"
                  @click="emit('payment-process', registration)"
                  class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-light-blue hover:bg-dark-blue focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-light-blue transition-all"
                >
