...
func (h *UserHandler) CreateUser(c echo.Context) error {
	// Get the raw request body to log it
	...
	// Replace the request body for binding
	...
	// Log the full user data being created
	log.Printf("Creating user with data: %+v", user)
	log.Printf("Contact: %+v", user.Contact)
	log.Printf("Address: %+v", user.Address)
	log.Printf("Medical Info: %+v", user.MedicalInformation)
	log.Printf("Personal Info: %+v", user.PersonalInformation)
	log.Printf("People Info: %+v", user.People)

	// Create user with transaction
	if err := h.repo.Create(&user); err != nil {
		log.Printf("CreateUser error: %v", err) // Detailed logging

		// Check if the error is related to email uniqueness constraint
